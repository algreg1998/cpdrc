...
//		$this->form_validation->set_rules('RepublicAct','RepublicAct','trim|strip_tags|xss_clean|required');
	...
	public function deleteviolation($id){
		...
		$logData = array(
					...
					'reasons' => 'Violation "'.$violation_info->name.'
					" was deleted',
					'update_by' => $this->session->userdata('user_id'),
					'action' => 'delete',
					'created_at' => date("Y-m-d h:i:sa"),
	...
	public function editcourt($id)
	{
		...
		if ($this->form_validation->run() !== FALSE) {
			...
			$logData = array(
						...
						'reasons' => 'Court "'.$chkr->court_name.'" was updated to "'.$this->input->post('court_name').'"',
						'update_by' => $this->session->userdata('user_id'),
						'action' => 'update',
						'created_at' => date("Y-m-d h:i:sa"),
						'status' => 'active'
	...
	public function editcell($id)
	{
		...
		if ($this->form_validation->run() !== FALSE) {
			...
			// }
			$this->session->set_flashdata('success_msg','Cell was successfully updated.');
			//echo $this->input->post('court_mun');
			...
			$logData = array(
						...
						'table_field' => 'id',
						'subject' => 'Update Cell',
						'reasons' => 'Cell "'.$chkr->cellNumber.'" was updated to "'.$this->input->post('cellNumber').'" ',
						'update_by' => $this->session->userdata('user_id'),
						'action' => 'update',
						'created_at' => date("Y-m-d h:i:sa"),
						'status' => 'active'
